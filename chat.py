    ...
    while 1 == 1:  # added loop here
        ...
        if choice == 1:  # entering choice in int
            ...
            while 1 == 1:  # loop added
                ...
                if option == 1:
                    while True:
                        salutation = raw_input("Mr/Miss:")  # salution
                        if len(salutation) == 0:
                            print "Re-enter"
                            continue
                        spy_name = raw_input("Enter  name:")  # spy_name
                        if len(spy_name) == 0:
                            print "Re-enter"
                            continue
                        age = int(raw_input("Enter age:"))  # spy_age
                        if age < 18:
                            print "Re-enter(above18)"
                            continue
                        rating = float(raw_input("Enter ratings:"))  # spy_rating
                        if rating < 3:
                            print "Re-enter(above 3)"
                            continue
                        # calling class into variable
                        frn = Name(salutation, spy_name, age, rating)
                        print "adding..."
                        print "New friend added to  friend list", '-' + frn.spy_name
